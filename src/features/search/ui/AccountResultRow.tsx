...
import type { SearchAccount } from '../model/types';
...
const AccountResultRow = ({ account, onToggleFollow }: AccountResultRowProps) => {
  return (
    <div className="flex items-center gap-3 px-4 py-3">
      <img src={profileDefault} alt="" className="size-11 shrink-0 rounded-full object-cover" />
      <div className="min-w-0 flex-1">
        <p className="text-body-2 text-text-primary truncate font-semibold">{account.name}</p>
        <p className="text-body-4 text-text-tertiary truncate">
          팔로우 {formatCount(account.followerCount)}명 · 게시물 {account.postCount}개
        </p>
      </div>
